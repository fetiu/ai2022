...
class Chromosome:
    ...
    def cal_fitness(self):# 적합도를 계산한다. 
        ...
        for next in self.genes:
            distance += k_map[now][next]
            now = next
        distance += k_map[now][0]
        self.fitness = MIN_DISTANCE/distance
        return self.fitness
...
def splice(slice, at, origin):
    other_slice = [x for x in origin if x not in slice]
    return other_slice[:at] + slice + other_slice[at:]
...
while population[0].cal_fitness() < 1:
    new_pop = []
